...
<head>
    ...
    <style>
        ...
        .dark-mode .offre-card { background: #232c3a !important; color: #e4e8ef; }
        .dark-mode .btn-modern { background: #003366 !important; color: #fff; }
        .dark-mode .footer { background: #15191f !important; }
        .content-section {
            background: #fff;
            padding: 32px;
            border-radius: 22px;
        ...
        h3 i, h5 i { color: #0099cc; animation: iconPop .6s; }
        @keyframes iconPop {
            0% { transform: scale(.7) rotate(-15deg);}
        ...
        .avatar:hover { box-shadow: 0 6px 22px #80d6ff80; }
        .profil-header { display: flex; align-items: center; margin-bottom: 1rem; }
        .profil-header .btn { margin-left: auto; }
        .btn-modern {
            border-radius: 27px;
            font-weight: 500;
        ...
        .badge-status {
            font-size: 1rem; padding: 0.55em 1.3em; border-radius: 17px;
            letter-spacing: .03em;
        ...
        .offre-card h5 { font-weight: 600; color: #006699; margin-bottom: 7px; display: flex; align-items: center; }
        .footer { background-color: #003366; color: white; }
        .section-anim { animation: fadeUp .6s .2s forwards; }
        .float-btn {
        ...
        .float-btn:hover { box-shadow:0 8px 40px #0099cc55; transform:scale(1.1);}
        @keyframes bounceIn { 0%{transform: scale(.6);} 60%{transform:scale(1.15);} 100%{transform:scale(1);} }
        /* Swiper carousel */
        .swiper { width: 100%; padding: 12px 0 34px 0;}
        .swiper-slide { background: #fff; border-radius: 14px; box-shadow:0 1px 8px #0099cc11; padding:20px;}
        @media (max-width: 900px) {
            .content-section { padding: 16px; }
            .offre-card { padding: 14px 9px;}
        }
        @media (max-width: 768px) {
            .profil-header { flex-direction: column; align-items: flex-start;}
            .avatar { margin-bottom: 14px;}
            .float-btn { bottom:12px; right:12px;}
        }
        /* Loader style */
        #loader {
            display:none; position:fixed;top:0;left:0;width:100vw;height:100vh;
            background:#fff8fcf2;z-index:9999;align-items:center;justify-content:center;
        }
    </style>
</head>
<body>
                ...
                <ul class="list-group list-group-flush shadow-sm">
                    <?php foreach ($candidatures as $c):
                        switch (strtolower($c['statut'])) {
                            case 'acceptée': $badge = 'success'; break;
                            case 'refusée':  $badge = 'danger'; break;
    ...
    toastr.success('Candidature envoyée avec succès !','Succès',{timeOut:2200,positionClass:'toast-bottom-right'});
    <?php endif; ?>
